    ...
    <section class="wrapper py-8 pb-32">
        ...
        <div class="flex gap-2 items-start bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 mb-6">
            ...
            <p class="text-xs text-blue-800">
                Data ini digunakan oleh <strong>sistem pakar</strong> untuk memfilter obat yang kontraindikasi dengan
                kondisi Anda.
                ...
                <ul class="text-xs text-red-700 space-y-1">
                    @foreach ($errors->all() as $error)
                        <li class="flex items-start gap-1.5">
                            <svg class="w-3.5 h-3.5 text-red-500 flex-shrink-0 mt-0.5" fill="currentColor"
                                viewBox="0 0 20 20">
                                <path fill-rule="evenodd"
        ...
        <form action="{{ route('profile.complete.store') }}" method="POST" class="flex flex-col gap-6">
            ...
            <div class="p-5 bg-white rounded-2xl shadow-sm">
                ...
                <div class="flex flex-col gap-4">
                    <div>
                        <label for="age" class="block text-sm font-semibold mb-2">Usia <span
                    ...
                    <div>
                        <label for="gender" class="block text-sm font-semibold mb-2">Jenis Kelamin <span
                                class="text-red-500">*</span></label>
                        <select name="gender" id="gender"
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary outline-none text-base appearance-none"
                            required>
            ...
            <div class="p-5 bg-white rounded-2xl shadow-sm">
                ...
                <p class="text-xs text-gray-500 mb-4">Pilih kondisi yang pernah atau sedang Anda alami. Biarkan "Tidak"
                    jika tidak ada.</p>
                            ...
                            <div class="flex gap-2 shrink-0">
                                <label class="inline-flex items-center cursor-pointer">
                                    <input type="radio" name="{{ $a['name'] }}" value="1"
                                        {{ old($a['name']) == '1' ? 'checked' : '' }}
                                        class="w-4 h-4 text-primary border-gray-300">
                                    <span class="ml-1 text-xs text-green-600">Ya</span>
                                </label>
                                <label class="inline-flex items-center cursor-pointer">
                                    <input type="radio" name="{{ $a['name'] }}" value="0"
                                        {{ old($a['name'], '0') == '0' ? 'checked' : '' }}
                                        class="w-4 h-4 text-primary border-gray-300">
                                    <span class="ml-1 text-xs text-gray-500">Tidak</span>
                                </label>
                            </div>
            ...
            <div class="fixed z-50 bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4">
                <div class="max-w-[425px] mx-auto">
                    <button type="submit"
                        class="w-full bg-primary text-white rounded-full py-4 font-bold text-base shadow-lg hover:bg-opacity-90 transition-all">
