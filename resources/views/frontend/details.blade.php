    ...
    @auth
      @if(count($safetyWarnings) > 0 || $pregnancyWarning || $ageWarning)
        <div class="flex flex-col gap-3 mb-4">
            ...
            <div class="bg-red-50 border-l-4 border-red-500 rounded-xl p-4">
              <div class="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p class="text-xs text-red-700 mb-2">Obat ini memiliki kontraindikasi dengan kondisi medis Anda:</p>
                  ...
                  <p class="text-xs text-red-600 mt-2 italic">Konsultasikan dengan apoteker sebelum membeli obat ini.</p>
                </div>
              </div>
            </div>
          @endif
          ...
          {{-- Kehamilan Warning --}}
          @if($pregnancyWarning)
            <div class="{{ $pregnancyWarning['level'] === 'danger' ? 'bg-red-50 border-red-500' : 'bg-yellow-50 border-yellow-500' }} border-l-4 rounded-xl p-4">
              <div class="flex items-start gap-3">
                <div class="flex-shrink-0 mt-0.5">
                  <svg class="w-5 h-5 {{ $pregnancyWarning['level'] === 'danger' ? 'text-red-500' : 'text-yellow-500' }}" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/>
                  </svg>
                </div>
                <div>
                  <p class="text-sm font-bold {{ $pregnancyWarning['level'] === 'danger' ? 'text-red-800' : 'text-yellow-800' }} mb-1">Peringatan Kehamilan</p>
                  <p class="text-xs {{ $pregnancyWarning['level'] === 'danger' ? 'text-red-700' : 'text-yellow-700' }}">{{ $pregnancyWarning['message'] }}</p>
                </div>
              </div>
            </div>
          @endif
