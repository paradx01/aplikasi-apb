<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <form action="{{ route('admin.medicine-recommendation.store') }}" method="POST" id="recommendation-form">
                @csrf
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6">
                            ...
                            <select name="disease_id" id="disease_id"
                                    class="w-full px-4 py-3 text-base border border-gray-300 dark:border-gray-600 rounded-lg
                        ...
                        <!-- Card: Profil Target Pasien (Rentang Umur) -->
                        <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6">
                            <h3 class="text-base font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                                <svg class="w-5 h-5 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <div class="space-y-5">
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                        Rentang Umur <span class="text-red-500">*</span>
                                    </label>
                                    ...
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                        Contoh: Anak 0–12, Dewasa 12–120.
                                    </p>
                                </div>
                            </div>
                        </div>

                        <!-- Card: Priority & Notes (global untuk relasi ini) -->
                        <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6">
                            <h3 class="text-base font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                                <svg class="w-5 h-5 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <div class="space-y-5">
                                <div>
                                    <label for="priority" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                        Prioritas Default <span class="text-red-500">*</span>
                                    </label>
                                    ...
                                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                        ...
                                        Nanti bisa diubah per produk jika perlu.
                                    </p>
                                </div>
                                ...
                                <div>
                                    ...
                                    <textarea name="notes" id="notes" rows="3"
                                              ...
                                              placeholder="Catatan khusus untuk rekomendasi penyakit ini (misal: gunakan bila keluhan dominan mual, dsb)...">{{ old('notes') }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="lg:col-span-1">
                        <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6 lg:sticky lg:top-6">
                            <div class="flex items-center justify-between mb-4">
                                ...
                                <span class="text-xs px-2 py-1 bg-indigo-100 dark:bg-indigo-900 text-indigo-700 dark:text-indigo-200 rounded-full font-semibold">
                            ...
                            <div class="space-y-2 max-h-[600px] overflow-y-auto pr-2" id="product-grid">
                                @foreach($products as $product)
                                    @php
                                        $checked = is_array(old('products')) && in_array($product->id, old('products', []));
                                    @endphp
                                    <div class="product-card border-2 rounded-lg p-3 cursor-pointer transition hover:shadow-md
                                                {{ $checked ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20' : 'border-gray-200 dark:border-gray-600' }}"
                                         data-product-name="{{ strtolower($product->name) }}">
                                        <label class="flex items-center gap-3 cursor-pointer">
                                            <input type="checkbox" name="products[]" value="{{ $product->id }}"
                                                   class="product-checkbox w-5 h-5 text-indigo-600 rounded focus:ring-indigo-500 flex-shrink-0"
                                                   {{ $checked ? 'checked' : '' }}
